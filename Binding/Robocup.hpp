...
private:
  ...
  //[Sol]
  mutable std::mutex whiteLinesMutex; //TODO: maybe use visionMutex instead
...
public:
  ...
  //[Sol]
  std::vector<Vision::Filters::WhiteLinesData> whitelines_data_accumulated;
  std::vector<Vision::Filters::WhiteLinesData> whitelines_data_single_frame;    
  std::vector<Vision::Filters::WhiteLinesData> stealWhiteLines();
  ...
  cv::Mat getApproachImg();
  cv::Mat getApproachImg(int width, int height); //[Sol]
  void drawDistortedWorldLineOnGround(cv::Mat& img, cv::Point2f p0, cv::Point2f p1, double scale_x, double scale_y, cv::Scalar color = cv::Scalar(0,255,0), int thickness = 2, double step = 0.2);
...
private:
  ...
  std::unique_ptr<std::vector<cv::Point2f>> detectedBallsSelf; //[Sol] for debug
  ...
  std::vector<Filters::WhiteLinesData> whiteLinesCopy; //[Sol]

  bool accumulateApproachVerbose;
  bool accumulateApproachVerboseOld;
  cv::Mat accumulatedApproachImg;
  cv::Point2f ballPosOnApproachAccumulatedImg;
  rhoban_utils::Angle ballAngleOnApproachAccumulatedImg;
  rhoban_utils::TimeStamp approachImgSavedAtTime;
  rhoban_utils::TimeStamp approachStartTime;
  int approachImgSavedImageNumber;
  int approachImgSavedApproachNumber;
  float pipelineFps;
  float pipelineFpsFiltered = 0;
  float moveSchedulerFps;
  float moveSchedulerFpsFiltered = 0;  
  void displaySpecialImagehandlers();
