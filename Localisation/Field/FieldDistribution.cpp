                   ...
                   hl_communication::WeightedPose* self_in_field, int nbParticles)
{
  ...
  {
    //Not enough data point exception when particle filter's particle count is too low
    return;
  }
...
void processClusters(std::map<int, PositionClusters>* pos_clusters, std::map<int, AngleClusters>* angle_clusters)
{
                 ...
                 std::map<int, PositionClusters>* pos_clusters, std::map<int, AngleClusters>* angle_clusters)
{
  ...
  std::vector<int> labels_counter(nbParticles, 0);


  for (int i = 0; i < nbParticles; i++)
  {
    labels_counter[labels.at<int>(i)] += 1;
  }

  for (int i = 0; i < nbParticles; i++)
  {
    if ((labels_counter[i] != 0) && (labels_counter[i]) < 3){
      continue;
    }
...
std::vector<hl_communication::WeightedPose> updateEM(const cv::Mat& positions, const cv::Mat& angles, int max_clusters)
{
  ...
  while ((posVarReduction > 0.5 || angleVarReduction > 0.5) && nbCluster <= max_clusters)
  {
    ...
    if (posVar == 0 || angleVar == 0)
    {
      std::cout << "Warning: posVar=" << posVar << ", angleVar=" << angleVar << std::endl;
