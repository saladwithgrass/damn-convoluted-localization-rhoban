...
grid_size_x = 8
grid_size_y = 7
square_size_m = 0.0574

USE_RATIONAL_MODEL = 1
...
class StereoCalibration(object):
    ...
    def read_images(self, cal_path):
        ...
        for i, fname in enumerate(images_right):
            ...
            img_l_s = cv2.imread(images_left[i])
            img_r_s = cv2.imread(images_right[i])

            # As on 2020, stereo calibration uses 720[540 images, but for wideangle_full calibration we need 1440x1080 images
            img_l = cv2.pyrUp(img_l_s)
            img_r = cv2.pyrUp(img_r_s)
        ...
        visUR = np.concatenate((sample_left, sample_left), axis=1)
        visR = np.concatenate((rectified_left, rectified_left), axis=1)
        vis = np.concatenate((cv2.pyrDown(visUR), cv2.pyrDown(visR)), axis=0)
        cv2.imshow("single cam calibration", vis)

        self.printMat("K",self.M1)
        self.printMat("D",self.d1)
        #M[row][col]
        print("\"center_x\" : " + str(self.M1[0][2]) + ",")
        print("\"center_y\" : " + str(self.M1[1][2]) + ",")
        print("\"focal_x\" : " + str(self.M1[0][0]) + ",")
        print("\"focal_y\" : " + str(self.M1[1][1]) + ",")

        print("\"radial_coeffs\" : [" + str(self.d1[0][0]) + "," + str(self.d1[0][1]) + "," + str(self.d1[0][4]) + "], //k1 k2 k3" )
        print("\"tangential_coeffs\" : [" + str(self.d1[0][2]) + "," + str(self.d1[0][3]) + "], //p1 p2")
        print("\"radial_coeffs_rational\" : [" + str(self.d1[0][5]) + "," + str(self.d1[0][6]) + "," + str(self.d1[0][7]) + "], //k4 k5 k6" )
