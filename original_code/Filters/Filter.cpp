...
Filter::Filter(const std::string& n, const Dependencies& dependencies)
  ...
  , warningExecutionTime(0.02) //[Sol] 0.01 on default single-threaded rhoban pipeline, 0.02 for multithreaded pipeline because some filters run in parallel and take more time
{
  _defaultRoi = false;
  setParameters();
  img_index = 0;
  ...
  cv::setMouseCallback(
      ...
      [](int event, int x, int y, int, void* param) -> void {
        ...
        else
        {
          ...
          //[Sol] draw assumed line width
          
          Eigen::Vector3d line_pos;
          int px0,py0, px1,py1, px2,py2, px3,py3;
          float dx,dy;
          filter->getCS().lineInfoFromPixel(ball_center_in_img,
                                                      &dx, &dy,
                                                      &px0, &py0,
                                                      &px1, &py1,
                                                      &px2, &py2,
          ...
          std::cout << "Line dx=" << dx <<", dy=" << dy << std::endl;

          cv::line(filter->cachedImg(), cv::Point(px0,py0), cv::Point(px1,py1), cv::Scalar(255,0,0), 2);
          cv::line(filter->cachedImg(), cv::Point(px2,py2), cv::Point(px3,py3), cv::Scalar(255,0,0), 2);
...
void Filter::runStep(UpdateType updateType)
{
  ...
  //std::cout << "Save image into " << getClassName() + "/" + std::to_string(img_index) + ".jpg" << std::endl;
  if (save)
  {

    mkdir(getClassName().c_str(), S_IRWXU | S_IRWXG | S_IROTH);
    cv::Mat& current_img = cachedImg();
    cv::imwrite(getClassName() + "/" + std::to_string(img_index) + ".jpg", current_img);
    std::cout << "Save image into " << getClassName() + "/" + std::to_string(img_index) + ".jpg" << std::endl;
...
void Filter::fromJson(const Json::Value& v, const std::string& dir_name)
{
  ...
  try
  {
    ...
    rhoban_utils::tryRead(v, "save", &save);
    // Read dependencies
