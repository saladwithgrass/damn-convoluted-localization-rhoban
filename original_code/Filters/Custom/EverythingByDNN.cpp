...
{
  importer = cv::dnn::createTensorflowImporter(model_path.c_str()); //4.3.0 readNetFromTensorflow
...
std::pair<int, double> EverythingByDNN::getClass(cv::Mat patch)
{
  ...
  cv::dnn::Blob in = cv::dnn::Blob::fromImages(normalized_patch); // 4.3.0 cv::dnn::blobFromImage

  net.setBlob(".img_placeholder", in); // 4.3.0 setInput(...)

  Benchmark::open("predict");
  net.forward(); // 4.3.0 forward(std::vector< std::vector< Mat > > &outputBlobs, const std::vector< String > &outBlobNames)
