...
private:
  ...
  cv::Mat extendedTopView;
  cv::Mat heatMap;
  cv::Mat heatMapDirectionsMask;  
  double gyroYawToOpponentGoal = 0; //in degrees, current value of IMU yaw when robot is facing opponent's goal
  bool gyroYawToOpponentGoalSetted = false;  
...
public:
  ...
  void importFromRhIO();
  void publishToRhIO();
  void publishExtendedTopViewToRhIO();
  ...
  std::vector<Localisation::WhiteLinesCornerObservation *> extractWhiteLinesCornerObservations(); //[Sol]
  ...
  ///[Sol] update heapMap data with observations if debug image streaming is enabled
  void updateHeatMap(const ObservationVector & obs);
  ...
  void redrawTopViewBody();
  void redrawTopViewHeader();

  //[Sol]
  cv::Mat getHeatMap(int width, int height);  
  ...
  //[Sol] export consistency step delta
  double stepDeltaScore; 
  int consistencyObsSize;
  int consistencyGoodObsCount, consistencyBadObsCount;
  double finalReprParticlePotential;
...
private:
  ...
  //[Sol]
  std::vector<Vision::Filters::WhiteLinesData> whitelines_data;
  double maxPotential;
  //[Sol] Get the best aviable potential on the filed _with current repr.particle orientation_ (for consistency calculation)
  double getMaxPotential( const std::vector<rhoban_unsorted::Observation<Localisation::FieldPosition> *> & obs ) ;
  double getGyroYawPotential();
