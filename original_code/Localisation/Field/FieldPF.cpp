...
std::map<FieldPF::ResetType, std::string> FieldPF::resetNames = { { ResetType::None, "None" },
                                                                  ...
                                                                  { ResetType::UniformDirected, "UniformDirected" }
                                                                  };
  ...
  : ParticleFilter(), resetType(ResetType::None), errorTols({ 8 }), resamplingRatio(0.0), tolDist(1), tolDiffAngle(15)
{
  ...
  rhioNode->newFloat("directedResetNoiseTheta")
      ->defaultValue(25)
      ->minimum(0)
      ->maximum(180)
      ->comment("uniform directed reset angle noise in [-x,x] [deg]");      
}
...
void FieldPF::applyReset(ResetType t)
{
  ...
  double mins[3] = {-Constants::field.field_length / 2 - Constants::field.border_strip_width_x,
                    -Constants::field.field_width / 2 - Constants::field.border_strip_width_y, 0};
  double maxs[3] = {Constants::field.field_length / 2 + Constants::field.border_strip_width_x,
                    Constants::field.field_width / 2 + Constants::field.border_strip_width_y, 360};      
  ...
  switch (t)
  {
    ...
    case UniformDirected:
    {
      initializeAtUniformRandomDirected(particles.size());
      break; 
    }
    default:
    {
                ...
                const cv::Scalar& color = cv::Scalar(255, 0, 255), int thickness = 1)
{
  ...
  //cv::circle(img, pos_img, 1, color, 1);  //[Sol] for debug
}
...
void FieldPF::initializeAtUniformRandomDirected(unsigned int particlesNb)
{
  //1. Initialise particles with zero direction
  double mins[3] = {-Constants::field.field_length / 2 - Constants::field.border_strip_width_x,
                    -Constants::field.field_width / 2 - Constants::field.border_strip_width_y, 0};
  double maxs[3] = {Constants::field.field_length / 2 + Constants::field.border_strip_width_x,
                    Constants::field.field_width / 2 + Constants::field.border_strip_width_y, 0};       
  ...
  //2. Rotate particles to current gyro yaw with some noise
  auto generator = rhoban_random::getRandomEngine();
  std::uniform_real_distribution<double> dirDistribution(-directedResetNoiseTheta, directedResetNoiseTheta);
  for (auto& p : particles)
  {
    p.first.rotate(Angle(gyroYawForUniformDirectedReset));
    p.first.rotate(dirDistribution(generator));
  }
}
