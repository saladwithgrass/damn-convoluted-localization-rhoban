...
public:
  struct Entry
  {
     ...
     * The image captured
     */
    cv::Mat img_l; //[Sol] for left cam (default)
    cv::Mat img_r; //[Sol] for right cam
    /**
  ...
  void initSession(const CameraState& cs, bool _is_stereo, const std::string& session_local_path = "");
  ...
  void pushEntry(const Entry& entry, bool _is_stereo);
