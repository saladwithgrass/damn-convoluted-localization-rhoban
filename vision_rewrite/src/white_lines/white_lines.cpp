...
int lineIntensityTreshold = 20;
double FIELD_WIDTH = 6.; // in meters
double FIELD_LENGTH  = 9.; // in meters
double GOAL_AREA_WIDTH = 3.;
int BORDER_STRIP_WIDTH_X = 15;
double largestCornerAngleDrift;
// Assumed width of lines to be detected (in pixels)
int LINE_WIDTH_BIRDVIEW = 20;
// Widnow size orthogonal to traverse direction
int WINDOW_HEIGHT_BIRDVIEW = 5;
...
float get_line_magnitude(cv::Point2i p1, cv::Point2i p2) {
    int dx = p2.x - p1.x;
    int dy = p2.y - p1.y;
    return sqrt(dx*dx + dy*dy);
}

float get_line_magnitude(Vec4i line) {
...
inline float get_point2line_distance(float px, float py, Vec4i line) {
    // just a more convenient wrapper
    return get_point2line_distance(px, py, line[0], line[1], line[2], line[3]);
}
...
float get_segnent2segment_distance(Vec4i line1, Vec4i line2) {
    // Get all possible distances between each dot of two lines and second line and return the shortest
    float dist1 = get_point2line_distance(line1[0], line1[1], line2);
    float dist2 = get_point2line_distance(line1[2], line1[3], line2);
    float dist3 = get_point2line_distance(line2[0], line2[1], line1);
    float dist4 = get_point2line_distance(line2[2], line2[3], line1);
...
inline float get_line2line_angle(Vec4i line1, Vec4i line2) {
    // just a convenient wrapper
    return get_line2line_angle(
...
inline void merge_two_segments(
        Vec4i line1,
        Vec4i line2,
        float* xr1, float* yr1, float* xr2, float* yr2) {
    // just a convenient wrapper
...
inline char get_line_intersection(
        Vec4i line1, Vec4i line2, float *i_x, float *i_y) {
...
void green_check_after_nms(
        ...
        float desired_green_coverage=0.33
) {
    nms_dest.setTo(0);

    // Checking for proper green color at left and right sides of a line
    for (int y = 0; y < img_to_draw.rows - WINDOW_HEIGHT_BIRDVIEW; y++) {
        for (int x = 0; x < img_to_draw.cols; x++) {
            if (nmsx.at<uchar>(y, x) > 0) {
                int val = 255;
                // Mark a line pixel blue before green check
                img_to_draw.at<Vec3b>(y, x) = Vec3b(255, 0, 0);
                img_to_draw.at<Vec3b>(y - 1, x) = Vec3b(255, 0, 0);
                img_to_draw.at<Vec3b>(y + 1, x) = Vec3b(255, 0, 0);
                img_to_draw.at<Vec3b>(y, x - 1) = Vec3b(255, 0, 0);
                img_to_draw.at<Vec3b>(y, x + 1) = Vec3b(255, 0, 0);

                // Filling data vector for central circle RANSAC
                circle_ransac_input.push_back(cv::Point2f(x, y));

                if (x >= img_to_draw.cols - LINE_WIDTH_BIRDVIEW * 3 / 2 - 1) break;  // not to hit the border
                ...
                if (gleft < desired_green_coverage * (float)LINE_WIDTH_BIRDVIEW * (float)WINDOW_HEIGHT_BIRDVIEW)
                    val = 0;  // 30% green coverage to the left of the line, TODO: remove hardcode
                if (gright < desired_green_coverage * (float)LINE_WIDTH_BIRDVIEW * (float)WINDOW_HEIGHT_BIRDVIEW)
                    val = 0;  // 30% green coverage to the right of the line, TODO: remove hardcode
                if (val > 0) nms_dest.at<uchar>(y, x) = val;
            }
        }
    }

    for (int x = 0; x < img_to_draw.cols - WINDOW_HEIGHT_BIRDVIEW; x++) {
        for (int y = 0; y < img_to_draw.rows; y++) {
            if (nmsy.at<uchar>(y, x) > 0) {
                ...
                int val = 255;
                if (y >= img_to_draw.rows - LINE_WIDTH_BIRDVIEW * 3 / 2 - 1) break;  // not to hit the border
                if (y < LINE_WIDTH_BIRDVIEW * 3 / 2 - 1) break;             // not to hit the border

                
                int gleft = 
                    countNonZero(green(Rect(x, y - LINE_WIDTH_BIRDVIEW * 3 / 2, WINDOW_HEIGHT_BIRDVIEW, LINE_WIDTH_BIRDVIEW)));
...
// For debug purposes
void draw_line_points_post_NMS(Mat& im, const Mat& nms) {
    for (int y = 0; y < im.rows; y++) {
        for (int x = 0; x < im.cols; x++) {
            if (nms.at<uchar>(y, x) > 0) {
                //Mark a line pixel with proper green by red pixel
...
void extend_lines(vector<Vec4i>& linesP_all, double extend_in_pix) {
...
void draw_meter_circles(Mat& im, int px_per_m) {
    ...
    for (int i = 1; i < (1 + 2 * FIELD_WIDTH); i++) {
        cv::circle(im, cv::Point2i(width / 2, height), (double)i * px_per_m, cv::Scalar(0, 0, 0), 1);
    }
...
void merge_line_segments(vector<Vec4i>& linesP_all) {
    bool changes_done_line = true;
    while (changes_done_line) {
        changes_done_line = false;
        for (size_t i = 0; i < linesP_all.size(); i++) {
            // for( size_t j = i+1; j < linesP_all.size(); j++ ) {
            Vec4i line_i = linesP_all[i];
            for (size_t j = 0; j < linesP_all.size(); j++) {
            Vec4i line_j = linesP_all[j];
                if ((i != j) &&
                        (get_line_magnitude(line_i) > 1.0) &&
                        (get_line_magnitude(line_j) > 1.0)) {

                    if ((get_segnent2segment_distance(line_i, line_j) < 10.0)
                            && (fabs(get_line2line_angle(line_i, line_j))*180.0 / M_PI < 5.0)) {
                        // This segments should be merged
                        float mx1, my1, mx2, my2;
                        merge_two_segments(line_i, line_j, 
                                &mx1, &my1, &mx2, &my2);
                        linesP_all[j][0] = mx1;
                        linesP_all[j][1] = my1;
                        linesP_all[j][2] = mx2;
...
void tune_white_lines(Mat source, Mat integralY, Mat green, CameraState* cs) {
    const char* winname = "tuning white lines";
    cv::namedWindow(winname);
    cv::createTrackbar(
            "lineIntensityTreshold",
            winname,
            &lineIntensityTreshold,
            1000
    );
    int field_width_int = 6000;
    cv::createTrackbar(
            "FIELD_WIDTH mm",
            winname,
            &field_width_int,
    ...
    char key = 0;
    while (key != 27) {
        FIELD_WIDTH = field_width_int / 1000.;
        FIELD_LENGTH = field_length_int / 1000.;
        GOAL_AREA_WIDTH = goal_width_int / 1000.;
        
        segment_white_lines(source, integralY, green, cs, winname);
        key = cv::waitKey(1);
...
void checkRefineAndDrawCornerCandidate(cv::Mat &im, 
    ...
    */
    ...
    double min_corner_segment_length_in_pixels = cs->getBirdviewPixelsInOneMeter()*0.5;
    if( (norm0>min_corner_segment_length_in_pixels) && (norm1>min_corner_segment_length_in_pixels) ) {
        //Refined corner is vaid

        circle(im, Point(C->x, C->y), 5, cv::Scalar(0,0,255), -1);
        //Drawing triangle base for better corner visibility
        line( im, Point(Ufa->x, Ufa->y), Point(Vfb->x, Vfb->y), cv::Scalar(125,255,0), 1); 

        //Drawing corner with conversion to L-shape
        //Angle from red to pink should be clockwise
        line( im, Point(Ufa->x, Ufa->y), Point(C->x, C->y), cv::Scalar(0,0,255), 2);
        line( im, Point(Vfb->x, Vfb->y), Point(C->x, C->y), cv::Scalar(0,128,255), 2);                
...
// Benchmark::open("Green check after NMS");
    ...
    std::vector<cv::Point2f> circle_ransac_input;
    green_check_after_nms(nmsx, nmsy, green, nms, im, circle_ransac_input);
    draw_line_points_post_NMS(im, nms);
...
// Benchmark::open("Probabilistic Hough Transform");
    ...
    // extend lines for more robust detection
    extend_lines(linesP_all, LINE_WIDTH_BIRDVIEW);
...
// Benchmark::open("Merging segments");
    ...
    merge_line_segments(linesP_all);
...
// Benchmark::open("Drawing segments");
    ...
    vector<Vec4i> linesP;
    for (size_t i = 0; i < linesP_all.size(); i++) {
        if (get_line_magnitude(linesP_all[i]) > 1.0) {
...
//------------------------------------------------------------------------------
    int px_per_m = cs->getBirdviewPixelsInOneMeter();
    draw_meter_circles(im, px_per_m);
...
// Benchmark::open("Checking lines for intersections");
    ...
    for (size_t i = 0; i < linesP.size(); i++) {
        for (size_t j = i + 1; j < linesP.size(); j++) {
            ...
            Vec4i la = linesP[i];
            Vec4i lb = linesP[j];
            char intersect = get_line_intersection(la, lb, &i_x, &i_y);
            if (intersect) {
                ...
                bool ps0_less_maxDist = (ps0.x < maxDist) && (ps0.y < maxDist);
                bool ps1_less_maxDist = (ps1.x < maxDist) && (ps1.y < maxDist);
                bool ps2_less_maxDist = (ps2.x < maxDist) && (ps2.y < maxDist);
                bool ps3_less_maxDist = (ps3.x < maxDist) && (ps3.y < maxDist);
                if (ps0_less_maxDist && ps1_less_maxDist &&// intersection point is farther than field size
                        ps2_less_maxDist && ps3_less_maxDist &&  // intersection point is farther than field size
...
// Benchmark::open("computeTransformations");
        ...
        for (int i = 0; i < loc_data_vector.size(); i++) {
            loc_data_vector[i].computeTransformations(cs, true);
        }
...
//------------------------------------------------------------------------------
        ...
        cv::line(im, unionSquareCenterPix + Point2i(-px_per_m / 2, px_per_m / 2), unionSquareCenterPix + Point2i(px_per_m / 2, px_per_m / 2),
                cv::Vec3b(0, 0, 255), 1);
        cv::line(im, unionSquareCenterPix + Point2i(px_per_m / 2, px_per_m / 2), unionSquareCenterPix + Point2i(px_per_m / 2, -px_per_m / 2),
                cv::Vec3b(0, 0, 255), 1);
        cv::line(im, unionSquareCenterPix + Point2i(px_per_m / 2, -px_per_m / 2), unionSquareCenterPix + Point2i(-px_per_m / 2, -px_per_m / 2),
