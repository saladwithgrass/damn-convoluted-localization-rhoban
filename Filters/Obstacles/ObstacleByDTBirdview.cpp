#include "Filters/Obstacles/ObstacleByDTBirdview.hpp"
...
ObstacleByDTBirdview::ObstacleByDTBirdview() : Filter("ObstacleByDTBirdview"){
  
}

void ObstacleByDTBirdview::setParameters()
{
  /*
...
double ObstacleByDTBirdview::my_fast_norm(double ax, double ay, double bx, double by) {
  return sqrt((ax - bx)*(ax - bx) + (ay - by)*(ay - by));
}

void ObstacleByDTBirdview::process()
{

  //clearObstaclesData();
  clearRobots();
  // Get names of dependencies
  const std::string & birdviewYName = _dependencies[0];
  const std::string & greenName = _dependencies[1];
  const std::string & integralYBirdviewName = _dependencies[2];
  // Import source matrix and update size
  const cv::Mat & birdviewYImg = *(getDependency(birdviewYName).getImg());  
  const cv::Mat & greenImg  = *(getDependency(greenName).getImg());
  const cv::Mat & inetgralYImg = *(getDependency(integralYBirdviewName).getImg());

  Vision::Utils::CameraState *cs = &getCS();

  Mat verbose;
  //verbose = birdviewYImg.clone();

  //Benchmark::open("inRange");
  cv::Scalar lowLimit = cv::Scalar(50);
  cv::Scalar highLimit = cv::Scalar(255);
  Mat white;
  inRange(birdviewYImg, lowLimit, highLimit, white);

  Mat safe_zone = white | greenImg;

  //std::vector<cv::Mat> channels;
  //split(birdviewImg, channels);
  //not_black = channels[1];
  //int blockSize = 15;
  //double C = 20;
  //adaptiveThreshold(channels[1], not_black, 125, CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY, blockSize, C);
  
  //inRange(birdviewImg, lowLimit, highLimit, not_black);
  //not_black = not_black | greenImg;  
  //Benchmark::close("inRange");  

  //erode(not_black, not_black, Mat(), Point(-1, -1), 1, 1, 1);
  //erode(not_black, not_black, Mat(), Point(-1, -1), 1, 1, 1);
  
  //dilate(not_black, not_black, Mat(), Point(-1, -1), 1, 1, 1);
  //dilate(not_black, not_black, Mat(), Point(-1, -1), 1, 1, 1);
