...
#define SHOW_BALL_RADIUS_DEBUG 0
...
void BallByII::process()
{
  ...
  const std::string& yDebugName = _dependencies[3];
  ...
  const cv::Mat& yDebug = *(getDependency(yDebugName).getImg());
  ...
  for (int y = 0; y * decimationRate < rows; y++)
  {
    for (int x = 0; x * decimationRate < cols; x++)
    {
      ...
      // Also in case of wrong camera calibration radius can be so large that it won't fit in float->int saturate_cast conversion 
      // Last example from debug session: "radius=9.01241772e+37"
      ...
      //bool rois_inside_img = Utils::isContained(boundaryPatch, size) && Utils::isContained(outputPatch, size); //Will throw SIGFPE in case of bad saturate_cast converting erraneous large float pixel coords to ints
      bool rois_inside_img = true;
      if (boundaryPatch.tl().x < 0 || boundaryPatch.br().x > size.width - 1 || boundaryPatch.tl().y < 0 || boundaryPatch.br().y > size.height - 1) rois_inside_img = false;
      if (outputPatch.tl().x < 0 || outputPatch.br().x > size.width - 1 || outputPatch.tl().y < 0 || outputPatch.br().y > size.height - 1) rois_inside_img = false;
  ...
  else
  {
    ...
    #if SHOW_BALL_RADIUS_DEBUG
    Benchmark::open("drawVerboseImage");  
    //[Sol] verbose debug
    cv::cvtColor(yDebug, img(), CV_GRAY2BGR);
    int verbose_step_pix = 64;
    for(int yi=0; yi <yImg.rows / verbose_step_pix; yi++) {
      for(int xi=0; xi <yImg.cols / verbose_step_pix; xi++) {
        int center_y = yi * verbose_step_pix;
        int center_x = xi * verbose_step_pix;
        float radius = radiusImg.at<float>(center_y, center_x) * inputSizeFactor; 
        cv::circle(img(), cv::Point2i(center_x, center_y), radius, cv::Scalar(255,0,255), 2); 
      }
    }
    Benchmark::close("drawVerboseImage");
    #endif
