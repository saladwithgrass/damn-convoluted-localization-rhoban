...
public:
  ...
  Utils::CameraState* getCameraState(bool isSlowStereo = false);
  void setCameraState(Utils::CameraState* csInit, bool isSlowStereo = false);
  ...
  bool benchmarkFromRobocup;
  int benchmarkDetailFromRobocup;
  
private:
  Utils::CameraState* cs;
  Utils::CameraState* cs_slow_stereo;
  ...
  //[Sol] thread QnD test
  std::thread* _stereoThread;

  //mutable std::mutex steroProcessingMutex;
  static std::atomic<bool> stereoDoProcessingOnce;  
  bool stopStereoThread = false;

  Filter* stereoImgProcFilter;

  mutable std::mutex steroProcessingMutex;
  std::condition_variable cv;  //Общая условная переменная, которая контролирует изменение зависимых флагов
  static bool stereoSourceReady;           //Выставляется, когда картинка готова для обработки
  static bool stereoProcessed;

  static ThreadPool threadPool;
  TimeStamp pipline_start_ts;
  bool first_step;
  int stereo_divider = 0;
